...
def test(args, net, segment, cluster, nice, test_loader, cmap):
    ...
    with Pool(40) as pool:
        for _, batch in prog_bar:
            ...
            save_all(args, ind, img, label, cluster_preds.argmax(dim=1), crf_preds, None, cmap, is_tr=True)

            # real-time print
            desc = f'{desc_nice}'
            desc_per_class = f'{nice.metric_dict_by_class}'
            prog_bar.set_description(desc)

    print(desc_per_class)
    print()
...
def test_linear_without_crf(args, net, segment, nice, test_loader, cmap):
    segment.eval()
    print("Test linear no cfr")
    prog_bar = tqdm(enumerate(test_loader), total=len(test_loader), leave=True)
    with Pool(40) as pool:
        for _, batch in prog_bar:
            # image and label and self supervised feature
...
def predict(args, net, segment, cluster, nice, test_loader, cmap):
    segment.eval()
    ...
    with Pool(40) as pool:
        for _, batch in prog_bar:
            ...
            save_all(args, ind, img, None, cluster_preds.argmax(dim=1), crf_preds, None, cmap, is_tr=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--just_imgs', default = False, type = bool)
    ...
    if 'small' in args.ckpt:
        ...
        args.dim=768
    elif 'visual' in args.ckpt:
        args.dim=768
    
    if 'visual' in args.ckpt:
